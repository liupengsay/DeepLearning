...
# data = generate_ctr_cvr_data(num_queries=1000, num_docs_per_query=50)
#
# # 查看数据的前几行
# print(data.head())
#
...
# print(roc_auc_score(y_ctr.values, data["query_doc_similarity"].values))
# print(roc_auc_score(y_cvr.values, data["query_doc_similarity"].values))

X = np.array([[random.uniform(0, 1) for _ in range(64)] for _ in range(500000)])
label = [sum(x) for x in X]
mid = sorted(label)[int(len(label) * 0.6)]
final = sorted(label)[int(len(label) * 0.9)]
y_ctr = [int(x) >= mid for x in label]
y_cvr = [int(x) >= final for x in label]  # CVR标签
print(sum(y_ctr)/len(y_ctr), sum(y_cvr)/len(y_cvr))
# 将数据划分为训练集和验证集
X_train, X_val, y_ctr_train, y_ctr_val, y_cvr_train, y_cvr_val = train_test_split(X, y_ctr, y_cvr, test_size=0.2,
                                                                                  random_state=42)

print(sum(y_ctr_train)/len(y_ctr_train), sum(y_cvr_train)/len(y_cvr_train))
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        val_loss = (loss_ctr_val + loss_cvr_val) / 2
