...
for epoch in range(num_epochs):
    ...
    query_terms_batch = query_terms_train.cuda()
    doc_terms_batch = doc_terms_train.cuda()
    user_id_batch = user_id_train.cuda()
    item_id_batch = item_id_train.cuda()
    X_batch = X_train.cuda()
    y_ctr_batch = y_ctr_train.cuda()
    y_cvr_batch = y_cvr_train.cuda()

    # 前向传播
    ctr_pred, cvr_pred = model(query_terms_batch, doc_terms_batch, user_id_batch, item_id_batch, X_batch)

    # 计算损失
    # print(ctr_pred.squeeze(), y_ctr_batch)
    ...
    query_terms_batch = query_terms_val.cuda()
    doc_terms_batch = doc_terms_val.cuda()
    user_id_batch = user_id_val.cuda()
    item_id_batch = item_id_val.cuda()
    X_batch = X_val.cuda()
    y_ctr_batch = y_ctr_val.cuda()
    y_cvr_batch = y_cvr_val.cuda()
    # 前向传播
    ctr_pred, cvr_pred = model(query_terms_batch, doc_terms_batch, user_id_batch, item_id_batch, X_batch)
